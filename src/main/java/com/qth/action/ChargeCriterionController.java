...
public class ChargeCriterionController extends BaseController {
    ...
    public CommonRsp insert(ChargeCriterion chargeCriterion, HttpSession session) {
        chargeCriterion.setCompany(getLoginUser(session).getOrg());
        int effect = chargeCriterionService.insertChargeCriterion(chargeCriterion);
        return dbEffect2Rsp(effect);
    }

    @RequestMapping(value = "update")
    public CommonRsp update(ChargeCriterion chargeCriterion, HttpSession session) {
        chargeCriterion.setCompany(getLoginUser(session).getOrg());
